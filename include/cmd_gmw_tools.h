...
typedef struct _console_frame_info_ {
	...
	int bwidth;
	int bhigh;
	/*pCGI->CFI.bhigh = pCGI->CFI.block_high * pCGI->row_num + pCGI->row_num + 1;
	pCGI->CFI.bwidth= pCGI->CFI.block_width * pCGI->col_num + pCGI->col_num *2+ 2;*/
...
typedef struct _console_graphics_info_ {
	...
	int cols ;
	/*pCGI->lines = pCGI->extern_up_lines + pCGI->extern_down_lines + pCGI->CFI.bhigh + pCGI->SLI.is_top_status_line + pCGI->SLI.is_lower_status_line+pCGI->draw_frame_with_row_no + 4;
	pCGI->cols = pCGI->extern_left_cols + pCGI->extern_right_cols + pCGI->CFI.bwidth + pCGI->draw_frame_with_col_no+1;*/
	/* 【下面允许添加你认为需要的值，这些值不能通过设置函数直接设置，只是为了方便程序中调用(不需要每次重复计算)
		 因为引入了 cmd_tgmw_tools.lib，为了保持两者的结构体一致，设置这部分大小为64字节（目前是pad），
		 如果需要添加相应内容，请保持添加内容+填充字节=64字节，
		   例如：int data1;
