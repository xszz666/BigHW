...
/* -----------------------------
	注意：本项(ENABLE_LIB_AAT_TOOLS)如果打开（置1）
				则本源文件中所有测试用例均会使用 lib_tgmw_tools.lib 中已实现的 st_args_analyse（即老师给出的已实现函数供参考）
		  本项(ENABLE_LIB_AATW_TOOLS)如果关闭（置0）
				则本源文件中所有测试用例均会使用 class_aat.cpp 中已实现的 args_analyse_tools（目前为空，需要自己实现）

		  通过本项的切换，可以灵活的在参考demo和自己实现之间做切换

	最终完成并交作业时，一定要做几件事：
		1、本项(ENABLE_LIB_AAT_TOOLS)必须置0
		2、从对应的 lib 目录中删除 lib_aat_tools.lib
----------------------------------------------------------------------------------------------------------------------------------------------------------- */
#define ENABLE_LIB_AAT_TOOLS			 0

#if ENABLE_LIB_AAT_TOOLS
